...
Training corpus: 4,186 design concept statements with five-dimension binary labels. Both the statements and the labels were generated by Anthropic's Claude under the author's direction, with structure and annotation rationales calibrated against interview transcripts from the author's design teaching practice. No statements were drawn directly from student work.

The full corpus is released in this repository under `corpus/`. See `corpus/README.md` for provenance details, file formats, and reproducibility instructions. The accompanying `aggregate-performance.md` documents per-classifier and aggregate accuracy, including what the numbers do and do not establish given that they were measured against the same generation pipeline that produced the training labels.
